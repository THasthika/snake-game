...
class Button():
    ...
    def __init__(self, text: str, font = None, size = 30, pos: Vec2 = Vec2(0, 0), color = TEXT_COLOR, anchor = AnchorType.CENTER, on_click = None, pad_x = 10, pad_y = 10):
    ...
    def update_surface(self):
        ...
        text_surface = self.font.render(self.text, True, self.color if not self.focused else (255 - self.color[0], 255 - self.color[1], 255 - self.color[2]))
        self.surface = pygame.Surface((text_surface.get_size()[0] + self.padding_x * 2, text_surface.get_size()[1] + self.padding_y *2))
        if self.focused:
